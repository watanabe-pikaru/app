<!DOCTYPE html>                   <!-- DOCTYPEは、「Document Type」の略で、「ドキュメントの型」という意味です。ここでは、文書がHTMLの型であることを宣言しています。 -->
  ...
  <head>                          <!-- headタグで囲まれたところ(head要素)にHTMLの文書の情報を書いていきます。 -->
    ...
    <title>記事追加</title>                                     <!-- タイトルは検索結果と、ブラウザのタブの上部に表示されます。 -->
        ...
        <div class="list">
            ...
            <?php     //データ保存
                ...
                if (isset($_POST['send']) && $_POST['post_content'] != "") {
                    ...
                    try {
                        $db->beginTransaction();
                        $stmt = $db->prepare("INSERT INTO posts (post_category, post_content) VALUES (?, ?)");
                        $stmt->execute(array($_POST["post_category"], $_POST["post_content"]));
                        $db->commit();
                    } catch (PDOException $e) {
                    // エラー発生時
                    $db->rollBack();
            ...
            <p>
                <?php echo he($page_message); ?>     <!-- 登録されたら表示 -->
            </p>
            <p class="attention">
                <?php echo he($page_error); ?>     <!-- エラーあるなら表示 -->
            </p>
            ...
            <form action="post_edit.php" method="post">
                <!-- 
                    ...
                    name=”post_category”という指定により、入力された値はpost_categoryという名前で送信されます。
                -->
                <div>
                ジャンル<br>
                <input type="text" name="post_category" size="30" value="<?php echo he($request["post_category"]); ?>">
                </div>
                <!--
                ...
                <div>
                名言<br>
                <textarea name="post_content" rows="10" cols="50"><?php echo he($request["post_content"]); ?></textarea>
                </div>
                <!--
